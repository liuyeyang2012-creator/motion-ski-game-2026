...
export function renderWelcome(root: HTMLElement, onStart: () => void): void {
  root.innerHTML = `<div class="screen welcome"><div class="brand">SNOWBREAK</div><div class="mountain-mark">▲</div><p class="eyebrow">工作间隙 · 动一动</p><h1>体感滑雪</h1><p class="lead">转动头部、活动身体<br>用轻缓动作穿过雪山赛道</p><button class="primary" data-testid="start">开始滑雪</button>${privacy}<p class="safety">开始前请清理身边障碍，身体不适请立即停止。</p></div>`
...
export function renderSetup(root: HTMLElement, defaults: SetupChoice, onSubmit: (choice: SetupChoice) => void): void {
  root.innerHTML = `<form class="screen setup"><p class="eyebrow">准备上雪道</p><h2>选择体感方式</h2><div class="choice-grid">
    <label class="choice"><input type="radio" name="playStyle" value="seated" ${defaults.playStyle === 'seated' ? 'checked' : ''}><strong>坐姿模式</strong><span>转头变道 · 抬头跳跃 · 低头躲避</span></label>
    <label class="choice"><input type="radio" name="playStyle" value="standing" ${defaults.playStyle === 'standing' ? 'checked' : ''}><strong>站立模式</strong><span>侧身 · 低头 · 抬手 · 安全下蹲</span></label></div>
...
export function renderResults(root: HTMLElement, result: ResultView, onReplay: () => void): void {
  const activity = result.playStyle === 'seated'
    ? `<p>转头变道 ${(result.motionCounts['turn-left'] ?? 0) + (result.motionCounts['turn-right'] ?? 0)} 次 · 跳跃 ${result.motionCounts['head-up'] ?? 0} 次</p><p>俯身 ${result.motionCounts['head-down'] ?? 0} 次 · ${Math.round(result.activeMs / 1000)} 秒</p>`
    : (() => {
